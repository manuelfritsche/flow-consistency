...
from semseg.models.utils import get_upsampling_weight
...
class fcn8s(nn.Module):
    def __init__(self, n_classes=21, learned_billinear=True, n_img=1):
        ...
        self.conv_block1 = nn.Sequential(
            nn.Conv2d(n_img * 3, 64, 3, padding=100),
            nn.ReLU(inplace=True),
            nn.Conv2d(64, 64, 3, padding=1),
            nn.ReLU(inplace=True),
            nn.MaxPool2d(2, stride=2, ceil_mode=True),
    ...
    def init_vgg16_params(self, vgg16, copy_fc8=True, n_img=1):
        blocks = [
            self.conv_block1,
            self.conv_block2,
            self.conv_block3,
        ...
        for idx, conv_block in enumerate(blocks):
            for l1, l2 in zip(features[ranges[idx][0] : ranges[idx][1]], conv_block):
                if isinstance(l1, nn.Conv2d) and isinstance(l2, nn.Conv2d):
                    if first_layer:
                        first_weight = l1.weight.repeat(1, n_img, 1, 1)
                        assert first_weight.size() == l2.weight.size()
                        l2.weight.data = first_weight.data
                        first_layer = False
                    else:
                        assert l1.weight.size() == l2.weight.size()
                        l2.weight.data = l1.weight.data
